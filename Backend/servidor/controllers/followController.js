...
const followUser = async (req, res) => {
  try {
    ...
    if (!req.actor?.mongo_id) {
      return res
        .status(409)
        .json({ error: "Usuario no sincronizado en este servicio" });
    }
    ...
    const followerExternalId =
      typeof follower.user_id !== "undefined" && follower.user_id !== null
        ? follower.user_id
        : follower._id;
    const followedExternalId =
      typeof followed.user_id !== "undefined" && followed.user_id !== null
        ? followed.user_id
        : followed._id;
  ...
  try {
    ...
    const followerExternalId =
      follower && follower.user_id !== undefined && follower.user_id !== null
        ...
        : follower_user_id;
    const followedExternalId =
      followed && followed.user_id !== undefined && followed.user_id !== null
        ? followed.user_id
        : followed_user_id;
...
const getFeed = async (req, res) => {
  try {
    ...
    // Obtengo los datos del usuario con el ObjectId
    const authors = await User.find({
      user_id: { $in: userIdNumberMapped },
    }).select("user_id username avatar_url email");

    const byUserId = new Map(authors.map((a) => [a.user_id, a]));
    ...
    const feed = publications.map((p) => ({
      ...p,
      author: byUserId.get(Number(p.user_id)) || null,
    }));
